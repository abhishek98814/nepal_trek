...
class RegisterView(generics.CreateAPIView):          # fix: CreateAPIView not createAPIView
    queryset = User.objects.all()
    serializer_class = RegisterSerializer            # fix: no () here
    permission_classes = [permissions.AllowAny]      # fix: permission_classes not permissions_classes

    def create(self, request, *args, **kwargs):
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)    # fix: added validation
...
class ProfileView(generics.RetrieveUpdateAPIView):   # fix: RetrieveUpdateAPIView not RetriveAPIView
    serializer_class = UserSerializer
    permission_classes = [permissions.IsAuthenticated]  # fix: IsAuthenticated not isAuthenticated
